const raicesSupabase = window.raicesSupabase || window.supabase.createClient(
  "https://tqtnffinhqbyesjdollk.supabase.co",
...
const copy = {
  es: {
    ...
    ordersTitle:"Mis pedidos", ordersIntro:"Consulta el estado y el resumen de tus compras.", ordersEmpty:"Todavía no tienes pedidos.", ordersSmall:"Aquí aparecerá el historial de tus compras.", refreshOrders:"Actualizar", shopNow:"Explorar tienda", favoritesTitle:"Favoritos", favoritesIntro:"Guarda productos para encontrarlos más rápido.", favoritesEmptyTitle:"Aún no tienes favoritos.", favoritesEmptyText:"Marca el corazón de un producto y aparecerá aquí.", orderError:"No pudimos cargar tus pedidos. Intenta nuevamente.", downloadDigital:"Descargar", digitalReady:"Descarga disponible",
...
function renderOrders(){
  ...
  list.innerHTML=state.orders.map(o=>{
    ...
    const shipment=o.shipment||null;
    const shippingHtml=shipment?`<div class="account-shipping-info"><strong>${escapeHtml(`${shipment.provider||"Shipping"} · ${shipment.service||""}`)}</strong><span>${escapeHtml(state.lang==="en"?"Shipping status":"Estado de Shipping")}: ${escapeHtml(shipment.shipment_status||"quoted")}</span>${shipment.tracking_number?`<span>${state.lang==="en"?"Tracking":"Tracking"}: ${escapeHtml(shipment.tracking_number)}</span>`:""}${shipment.tracking_url?`<a href="${escapeHtml(shipment.tracking_url)}" target="_blank" rel="noreferrer">${state.lang==="en"?"Track package":"Rastrear paquete"}</a>`:""}</div>`:"";
    return `<article class="account-order-card"><div class="account-order-top"><div><small>${o.source==="nurai_replacement"?(state.lang==="en"?"Replacement":"Reemplazo"):(state.lang==="en"?"Order":"Pedido")}</small><h3>#${escapeHtml(o.order_number||"")}</h3></div><span class="account-order-status ${statusClass}">${escapeHtml(status)}</span></div><div class="account-order-meta"><span>${escapeHtml(created)}</span><strong>${moneyFromCents(o.total_cents)}</strong></div>${items?`<p>${items}</p>`:""}${shippingHtml}${o.manage_token?`<p><a class="account-manage-order" href="/manage-order.html?token=${encodeURIComponent(o.manage_token)}">${state.lang==="en"?"Manage order":"Gestionar pedido"}</a></p>`:""}</article>`;
  }).join("");
}
