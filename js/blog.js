(function(){
  ...
  async function subscribe(e){e.preventDefault();const form=e.currentTarget,msg=form.querySelector('.blog-subscribe-message'),email=form.email.value.trim();msg.dataset.state='';msg.textContent=B.lang()==='en'?'Sending...':'Enviando...';try{const res=await fetch('/.netlify/functions/save-interest',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,name:'Lector del Blog',source:'blog_newsletter',consent:true,language:B.lang()})});if(!res.ok)throw new Error();form.reset();msg.dataset.state='ok';msg.textContent=B.lang()==='en'?'Thank you. You are now part of the Raíces community.':'Gracias. Ya formas parte de la comunidad Raíces.';}catch{msg.dataset.state='error';msg.textContent=B.lang()==='en'?'We could not save your email. Please try again.':'No pudimos guardar tu correo. Intenta de nuevo.';}}
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    window.addEventListener('raices:languageChanged',()=>{state.filter='all';applyHero();categories();render();});
