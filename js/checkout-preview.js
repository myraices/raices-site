(function(){
 ...
 const items=summary.items||[], cfg=window.RAICES_STORE_CONFIG||{}, rules=cfg.ORDER_RULES||{}; let zones=[], deliveryConfigReady=false, freeDeliveryEnabled=true, freeDeliveryActive=true, freeDeliveryThreshold=Number(rules.freeDeliveryThreshold??100), freeDeliveryStartDate="", freeDeliveryEndDate="";
 ...
 function render(){const data=save(),zip=zipNorm(data.zip),zone=digitalOnly?{name:'Entrega digital',fee:0}:(addressVerified&&zip.length===5?zoneFor(zip):null),subtotal=Number(summary.subtotal||0),physicalSubtotal=items.filter(i=>!isDigitalItem(i)).reduce((sum,i)=>sum+Number(i.lineTotal||((i.qty||0)*Number(i.price||0))),0),freeAt=freeDeliveryActive?Number(freeDeliveryThreshold||0):-1,qualifies=hasPhysicalItems&&freeDeliveryActive&&(freeAt===0||physicalSubtotal>=freeAt),delivery=digitalOnly?0:(zone&&items.length?(qualifies?0:Number(zone.fee||0)):0);
  ...
  const left=Math.max(0,freeAt-physicalSubtotal);freeDeliveryProgress.textContent=digitalOnly?'Los productos digitales no tienen cargo de delivery.':(!freeDeliveryEnabled?'':!freeDeliveryActive?(freeDeliveryStartDate?`Delivery gratis disponible desde ${freeDeliveryStartDate}.`:''):freeAt===0?'Delivery gratis en todas las compras físicas.':qualifies?'Has desbloqueado delivery gratis.':`Agrega ${money(left)} más en productos físicos para delivery gratis.`);
 ...
 if(payButton)payButton.addEventListener('click',async()=>{
  ...
  try{
   ...
   const pendingCheckout={id:body.checkoutId,checkoutId:body.checkoutId,environment:body.environment,createdAt:new Date().toISOString()};sessionStorage.setItem('raices_pending_order',JSON.stringify(pendingCheckout));localStorage.setItem('raices_pending_order',JSON.stringify(pendingCheckout));
