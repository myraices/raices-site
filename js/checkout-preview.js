(function(){
 ...
 const items=summary.items||[], cfg=window.RAICES_STORE_CONFIG||{}, rules=cfg.ORDER_RULES||{}; let zones=[], deliveryConfigReady=false;
 ...
 async function loadDeliveryConfig(){
  ...
  try{const res=await fetch('/.netlify/functions/delivery-config',{cache:'no-store'});const body=await res.json().catch(()=>({}));if(!res.ok||!Array.isArray(body.zones))throw new Error(body.error||'DELIVERY_CONFIG_UNAVAILABLE');zones=normalizeZones(body.zones);deliveryConfigReady=true;}catch(err){console.error('Raíces checkout delivery configuration error',err);zones=[];deliveryConfigReady=false;}
 }
