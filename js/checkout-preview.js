(function(){
 ...
 const items=summary.items||[], cfg=window.RAICES_STORE_CONFIG||{}, rules=cfg.ORDER_RULES||{}; let zones=[], deliveryConfigReady=false, freeDeliveryEnabled=true, freeDeliveryThreshold=Number(rules.freeDeliveryThreshold??100);
 ...
 async function loadDeliveryConfig(){
  ...
  try{const res=await fetch('/.netlify/functions/delivery-config',{cache:'no-store'});const body=await res.json().catch(()=>({}));if(!res.ok||!Array.isArray(body.zones))throw new Error(body.error||'DELIVERY_CONFIG_UNAVAILABLE');zones=normalizeZones(body.zones);freeDeliveryEnabled=body.freeDeliveryEnabled!==false;const remoteThreshold=Number(body.freeDeliveryThreshold);if(Number.isFinite(remoteThreshold)&&remoteThreshold>=0)freeDeliveryThreshold=remoteThreshold;deliveryConfigReady=true;}catch(err){console.error('Raíces checkout delivery configuration error',err);zones=[];deliveryConfigReady=false;}
 }
 ...
 function render(){const data=save(),zip=zipNorm(data.zip),zone=digitalOnly?{name:'Entrega digital',fee:0}:(addressVerified&&zip.length===5?zoneFor(zip):null),subtotal=Number(summary.subtotal||0),physicalSubtotal=items.filter(i=>!isDigitalItem(i)).reduce((sum,i)=>sum+Number(i.lineTotal||((i.qty||0)*Number(i.price||0))),0),freeAt=freeDeliveryEnabled?Number(freeDeliveryThreshold||0):-1,qualifies=hasPhysicalItems&&freeDeliveryEnabled&&(freeAt===0||physicalSubtotal>=freeAt),delivery=digitalOnly?0:(zone&&items.length?(qualifies?0:Number(zone.fee||0)):0);
  ...
  const left=Math.max(0,freeAt-physicalSubtotal);freeDeliveryProgress.textContent=digitalOnly?'Los productos digitales no tienen cargo de delivery.':(!freeDeliveryEnabled?'':freeAt===0?'Delivery gratis en todas las compras físicas.':qualifies?'Has desbloqueado delivery gratis.':`Agrega ${money(left)} más en productos físicos para delivery gratis.`);
  ...
  const msg=checkoutDeliveryMessage;if(!items.length){msg.dataset.state='error';msg.textContent='Tu carrito está vacío.'}else if(digitalOnly){msg.dataset.state='ok';msg.textContent='Producto digital: no requiere dirección ni tiene cargo de delivery.'}else if(!deliveryConfigReady){msg.dataset.state='idle';msg.textContent='Cargando cobertura de delivery…'}else if(!addressVerified){msg.dataset.state='idle';msg.textContent='Selecciona primero una dirección válida de Google para confirmar la cobertura.'}else if(!zone){msg.dataset.state='error';msg.textContent=`La dirección seleccionada (ZIP ${zip}) está fuera de la cobertura configurada.`}else{msg.dataset.state='ok';msg.textContent=`Cobertura confirmada: ${zone.name}. ${qualifies?'Delivery gratis.':`Delivery estimado ${money(delivery)}.`}`}
 ...
 if(payButton)payButton.addEventListener('click',async()=>{
  ...
  if(!digitalOnly&&!deliveryConfigReady){paymentMessage('No se pudo cargar la cobertura de delivery. Recarga la página e intenta nuevamente.','error');return;}
  ...
  try{
   ...
   const pendingOrder={id:body.orderId,orderNumber:body.orderNumber,environment:body.environment,createdAt:new Date().toISOString()};sessionStorage.setItem('raices_pending_order',JSON.stringify(pendingOrder));localStorage.setItem('raices_pending_order',JSON.stringify(pendingOrder));
   window.location.assign(body.checkoutUrl);
  }catch(err){console.error('Square checkout error',err);const messages={EMPTY_CART:'Tu carrito está vacío.',DELIVERY_OUTSIDE_COVERAGE:'La dirección está fuera de cobertura.',ADDRESS_NOT_VERIFIED:'Selecciona una dirección de entrega completa de las sugerencias de Google.',DELIVERY_DATA_INCOMPLETE:'Completa el teléfono y todos los datos de entrega.',PRODUCT_NOT_AVAILABLE:'Uno de los productos ya no está disponible.',INSUFFICIENT_STOCK:'No hay inventario suficiente.',LIVE_SALES_DISABLED:'Las ventas reales todavía no están habilitadas.',SQUARE_CONFIGURATION_MISSING:'Falta completar la configuración de Square.',DELIVERY_CONFIG_UNAVAILABLE:'No se pudo cargar la configuración de delivery. Intenta nuevamente.',TAX_RULE_NOT_CONFIGURED:'No hay una regla activa de Sales Tax para el estado de entrega. Revisa Configuración → Pagos en NURAI.',SQUARE_TAX_CALCULATION_FAILED:'Square no pudo validar el cálculo del tax. Intenta nuevamente.',TAX_CALCULATION_MISMATCH:'El cálculo fiscal no coincidió con Square y el pago fue bloqueado por seguridad.',DIGITAL_TAX_REVIEW_REQUIRED:'Este producto digital todavía está en revisión fiscal.',DIGITAL_FILE_MISSING:'Este producto digital todavía no tiene su archivo PDF cargado. Intenta nuevamente más tarde.',CHECKOUT_UNAVAILABLE:'No se pudo iniciar el pago. Intenta nuevamente.'};paymentMessage(messages[err.message]||messages.CHECKOUT_UNAVAILABLE,'error');payButton.disabled=false;payButton.textContent='CONTINUAR AL PAGO';}
 });
 (async()=>{await loadDeliveryConfig();render();hydrateFromAccount();loadDefaultAddress();if(!digitalOnly)initAddressAutocomplete();})();
})();
