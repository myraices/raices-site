(function(){
  'use strict';
  var content={};
  var previewMode=new URLSearchParams(window.location.search).get('cmsPreview')==='1';
  if(previewMode)document.documentElement.classList.add('cms-preview-mode');
  ...
  async function load(){apply();if(previewMode)return;try{var client=window.raicesSupabase;if(!client)return;var result=await client.from('site_content_public').select('content_key,content,sort_order').order('sort_order');if(result.error)throw result.error;(result.data||[]).forEach(function(row){if(row&&row.content_key&&row.content)content[row.content_key]=row.content});window.RAICES_SITE_CONTENT=content;apply()}catch(err){console.warn('[Raíces CMS] Se usará el contenido incorporado.',err&&err.message?err.message:err)}}
  window.addEventListener('message',function(event){if(!previewMode)return;var allowed=['https://admin.myraices.com','http://localhost:5173','http://localhost:8888'];if(allowed.indexOf(event.origin)===-1)return;var data=event.data||{};if(data.type!=='raices:cms-preview'||data.section!=='hero'||!data.content)return;content.hero=data.content;window.RAICES_SITE_CONTENT=content;applyHero(content.hero);});
