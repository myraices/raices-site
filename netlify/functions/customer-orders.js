...
exports.handler=async(event)=>{
  ...
  try{
    ...
    const shipments=await serviceRequest(`order_shipments?select=order_id,shipment_number,provider,service,shipment_status,tracking_number,tracking_url,estimated_days,delivered_at,created_at&order_id=in.(${ids.join(',')})&order=shipment_number.desc,created_at.desc`);
    const shipmentMap=new Map(); for(const s of shipments){if(!shipmentMap.has(s.order_id))shipmentMap.set(s.order_id,s);}
