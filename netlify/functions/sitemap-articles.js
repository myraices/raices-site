...
exports.handler=async()=>{
  try{
    ...
    const r=await fetch(`${url}/rest/v1/blog_articles_public?select=slug,published_at&order=published_at.desc`,{
    ...
    const urls=(rows||[])
      .filter(x=>x.slug)
      .map(x=>{
        const lastmod=String(x.published_at||'').slice(0,10);
        return `  <url><loc>${BASE}/blog/${esc(encodeURIComponent(x.slug))}</loc>${lastmod?`<lastmod>${esc(lastmod)}</lastmod>`:''}</url>`;
      })
      .join('\n');
    return{
      statusCode:200,
